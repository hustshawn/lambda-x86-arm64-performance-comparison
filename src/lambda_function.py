...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Processing operation: {operation} with params: {operation_params}")
        
        # Process the workload with metrics collection
        with MetricsContext(_metrics_collector, operation, 
                          data_size=data_size, iterations=iterations, 
...
def validate_operation_parameters(operation: str, parsed_event: Dict[str, Any]) -> Optional[str]:
    """
    Validate operation-specific parameters.
    ...
    try:
        if operation == 'sort_intensive':
            ...
            if not isinstance(memory_size_mb, int) or memory_size_mb < 1 or memory_size_mb > 100:
