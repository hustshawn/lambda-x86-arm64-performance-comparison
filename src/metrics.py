...
class PerformanceMetrics:
    ...
    def stop_timer(self, operation: str) -> float:
        ...
        execution_time = (end_time - self.start_times[operation]) * 1000  # Convert to ms
    ...
    def record_operation_metrics(self, operation: str, data_size: int = 0, 
                               iterations: int = 1, cold_start: bool = False,
        ...
        metrics = {
            ...
            'operation': operation,
            'execution_time_ms': self.metrics_data.get(operation, {}).get('execution_time_ms', 0.0),
        ...
        logger.info(f"Recorded metrics for {operation}: {json.dumps(metrics, indent=2)}")
        return metrics
    
    def send_cloudwatch_metrics(self, operation: str, namespace: str = 'Lambda/PerformanceComparison') -> bool:
        """
        ...
        try:
            ...
            if metric_data:
                ...
                logger.info(f"Successfully sent {len(metric_data)} metrics to CloudWatch")
                return True
            else:
...
class MetricsContext:
    ...
    def __init__(self, metrics: PerformanceMetrics, operation: str, 
                 data_size: int = 0, iterations: int = 1, cold_start: bool = False):
        """
...
def create_metrics_collector(architecture: str = None, function_name: str = None) -> PerformanceMetrics:
    """
